...
(define text-mode-cmd-map
   (let ([map (make-keymap 'text-mode-map)])
      ...
      (bind-key map "j" (lambda () (move-down-line)))
...
(define text-mode-vis-linewise-map
   (let ([map (make-keymap 'text-mode-map)])
      (bind-key map "<Esc>" text-mode-cmd)
      (bind-key map "x" (lambda () (mark-delete) (text-mode-cmd)))
      (bind-key map "l" (lambda () (move-down-line)))
      (bind-key map "j" (lambda () (move-down-line) (move-line-end)))
      (bind-key map "k" (lambda () (move-up-line) (move-line-end)))
