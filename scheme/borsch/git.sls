...
(define git-branch-name
   (lambda ()
      (let ([out (process-get-output (format "git -C ~a rev-parse --abbrev-ref HEAD 2> /dev/null" (current-cwd))) ])
         (string-remove-nl (list-ref out 1)) )))

(define git-short-status-string
   (lambda ()
      (let ([out (process-get-output (format "git -C ~a status 2> /dev/null" (current-cwd)))]
